!( function( $, _undefined ) {
	...
	var USWooCommerce = function() {
		...
		if ( this.isCart() ) {
			...
			this.$cartQuantity = $( '.w-cart-quantity', this.$cart );
	...
	$.extend( USWooCommerce.prototype, {
		...
		_updateCart: function() {
			if ( this.$cart.hasClass( 'opened' ) ) {
				this.$cart.removeClass( 'opened' );
			}
			var miniCartAmount = $( '.us_mini_cart_amount:first', this.$cart ).text();

			if ( miniCartAmount !== _undefined ) {
				miniCartAmount = ( miniCartAmount + '' ).match( /\d+/g );
				this.$cartQuantity.html( miniCartAmount > 0 ? miniCartAmount : '0' );
				this.$cart[ miniCartAmount > 0 ? 'removeClass' : 'addClass' ]( 'empty' );

			} else {
				// fallback in case our action wasn't fired somehow
				var total = 0;
				$( '.quantity', this.$cart ).each( function() {
				...
				this.$cartQuantity.html( total > 0 ? total : '0' );
				this.$cart[ total > 0 ? 'removeClass' : 'addClass' ]( 'empty' );
			}
		},
