...
if ( ! function_exists( 'us_checkout_page_content' ) ) {
	...
	function us_checkout_page_content( $content ) {
		...
		if (
			...
			AND $checkout_page_ID = get_option( 'woocommerce_checkout_page_id' )
			AND $checkout_page_POST = get_post ( $checkout_page_ID )
			AND strpos( $checkout_page_POST->post_content, "[us_checkout_" ) !== FALSE
		) {
