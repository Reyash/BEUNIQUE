...
if ( ! function_exists( 'us_iterate_queries' ) ) {
	...
	function us_iterate_queries( $query, $callback, $max_num_entries = 9999, $query_limit = 100 ) {
		...
		$query = us_strtolower( $query );
		if ( ( $pos = strpos( $query, ' limit ' ) ) !== FALSE ) {
		...
		if ( $is_empty_result ) {
			...
			if ( ! empty( $group_posts_types['us_content_template'] ) ) {
				...
				$query = "
					...
					WHERE
						tm.meta_value IN( '" . implode( "','", $group_posts_types['us_content_template'] ) . "' )
						AND tm.meta_key IN( '" . implode( "','", array_keys( $usage_meta_keys ) ) . "' )
				";

				// Iterate queries
				us_iterate_queries(
					$query,
					function ( $items ) use ( &$results, $group_posts_types, $usage_meta_keys ) {
						foreach ( $items as $item ) {
							foreach ( $group_posts_types['us_content_template'] as $id ) {
								if (
									...
									$result .= $item->name . ' > ';
									$result .= us_translate( $usage_meta_keys[ $item->meta_key ] ) . '</strong>';
